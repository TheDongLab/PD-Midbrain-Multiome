# =============================================================================
# Step 02 - QC filtering, normalization, Harmony integration, WNN clustering
# -----------------------------------------------------------------------------
...
# Outputs : <DIR_FINALWNN>/merged_seurat2.rda, merged_seurat4.rda,
...
METADATA_XLSX <- "/mnt/data/projects/donglab/ming2024/ASAP_scATAC_2024/metadata/2023-07-26-midbrain-scATAC-metadata.xlsx"

# Input merged object from step 01.
load("./01-Clusterings4/merged_seurat.rda")
...
# Canonical marker genes used for annotation throughout.
markerGenes <- c(
  "ENO2", "RBFOX3", "NEFM", "NEFL",                 # pan-neuronal
  "TH", "SLC6A3", "SLC18A2", "GIRK2", "DAT",        # DA neurons
  "SLC17A6", "SLC17A7", "GRIN1", "GLS",             # glutamatergic
  "SLC32A1", "GAD1", "GAD2",                        # GABAergic
  "MOBP", "PLP1", "MBP",                            # oligodendrocytes
  "AQP4", "GFAP",                                   # astrocytes
  "VCAN", "OLIG1", "OLIG2",                         # OPCs
  "CLDN5", "FLT1", "ENG",                           # endothelial
  "PDGFRB",                                         # pericytes
  "ABCA9",                                          # fibroblasts
  "CX3CR1", "P2RY12", "ARHGAP15",                   # microglia
  "MRC1", "CD163", "S100A4", "PLAC8",               # macrophages
  "CD8A", "CD96",                                   # CD8 T cells
  "PTBP1")

# WNN-based annotation.
Idents(merged_seurat3) <- merged_seurat3$wsnn_new_res.0.2
merged_seurat3 <- RenameIdents(merged_seurat3, "4" = "Astrocytes")
merged_seurat3 <- RenameIdents(merged_seurat3, "0" = "Oligodendrocytes", "1" = "Oligodendrocytes", "2" = "Oligodendrocytes", "3" = "Oligodendrocytes", "6" = "Oligodendrocytes", "7" = "Oligodendrocytes", "9" = "Oligodendrocytes", "12" = "Oligodendrocytes", "13" = "Oligodendrocytes")
merged_seurat3 <- RenameIdents(merged_seurat3, "8" = "OPCs")
merged_seurat3 <- RenameIdents(merged_seurat3, "11" = "Endothelial cell")
merged_seurat3 <- RenameIdents(merged_seurat3, "5" = "Microglia")
merged_seurat3 <- RenameIdents(merged_seurat3, "10" = "Neurons")
merged_seurat3$celltype.WNN <- Idents(merged_seurat3)

## ---- TSS-enrichment based iterative refinement ------------------------------
merged_seurat3$TSS.enrichment2 <- merged_seurat3$TSS.enrichment
merged_seurat3$TSS.enrichment2[merged_seurat3$TSS.enrichment2 > 10] <- 10

aa <- plotTSSEnrichmentDistribution(merged_seurat3, tssKey = "TSS.enrichment", rnaKey = "nCount_RNA",
                                    cellTypeKey = "wsnn_new_res.0.2", zscoreThreshold = -1.5)
merged_seurat4 <- aa$filteredSeuratObj
plotList <- aa$plotList
thresholdList <- as.data.frame(aa$thresholdList)
pdf(file.path(DIR_FINALWNN, "TSSvsRNAcounts.pdf"), width = 8, height = 8); print(plotList); dev.off()

p2 <- plotTSSEnrichmentRidges(merged_seurat4, groupBy = "celltype.WNN", fillBy = "celltype.WNN",
                              alpha = 0.6, legendPosition = "right", titleSize = 14, axisSize = 12,
                              annotationSize = 3, annotationNudge = 0.1, ridgeScale = 6)
pdf(file.path(DIR_FINALWNN, "UMAP.Clustering.TSSEnrichment.res02.after.pdf"), width = 15, height = 10); print(p2); dev.off()

for (thr in c(2, 3, 4)) {
  merged_seurat0 <- subset(x = merged_seurat4, subset = TSS.enrichment2 >= thr)
  p <- plotTSSEnrichmentRidges(merged_seurat0, groupBy = "celltype.WNN", fillBy = "celltype.WNN",
                               alpha = 0.6, legendPosition = "right", titleSize = 14, axisSize = 12,
                               annotationSize = 3, annotationNudge = 0.1, ridgeScale = 6)
  pdf(file.path(DIR_FINALWNN, paste0("UMAP.Clustering.TSSEnrichment.res02-TSS", thr, ".pdf")), width = 15, height = 10)
  print(p); dev.off()
}

p3 <- DimPlot(merged_seurat4, reduction = "wnn.umap_new", group.by = "celltype.WNN", label = TRUE, label.size = 2.5, repel = TRUE) + ggtitle("WNN")
pdf(file.path(DIR_FINALWNN, "UMAP.Clustering.Manual.WNN_res0.2_new.after.pdf"), width = 8, height = 5); print(p3 & theme(plot.title = element_text(hjust = 0.5))); dev.off()
...
meta$celltype.WNN.after.combine.update[which(meta$celltype.WNN.after.combine.update == "Oligodendrocytes_Neurons")] <- "Oligodendrocytes"
