...
[03] Cell-type ATAC peak calling (MACS)
      ...
      ├──────────────────┬──────────────────┐
      ▼                  ▼                  ▼
[04] cis-caQTL          [05] cis-eQTL       [08] Diagnosis DEGs (NEBULA)
      │                  │                  │
      └────────┬─────────┘                  │
               ▼                            │
         [06] Colocalization                │
              (caQTL + eQTL + PD GWAS)      │
               │                            │
               ▼                            │
         [07] Cell-type QTL enrichment      │
              of PD heritability (S-LDSC)   │
                                            │
[09] Peak–gene cis links (scMultiMap)       │
                                            │
[10] Cell-type GRNs (SCENIC+)               │
      │                                     │
      └──────────────┬──────────────────────┘
                     ▼
              [11] eRegulon enrichment
                   in diagnosis DEG ranks
```

**Upstream (01–03)** defines cell types and the accessible peak universe.
**QTL / GWAS modules (04–06)** ask which regulatory variants and cell types
contribute to PD risk. **DEG and linking (08–09)** and **GRN / eRegulon
(10–11)** connect disease-associated expression changes to candidate enhancers
...
| Step | Script | Biological aim |
|------|--------|----------------|
| 04 | `caQTL-preprocess.R` | Pseudobulk chromatin accessibility by donor and cell type; map cis-caQTLs (FastQTL) |
| 05 | `eQTL-preprocess.R` | Pseudobulk gene expression by donor and cell type; map cis-eQTLs (FastQTL) |
| 06 | `moloc-coloc.R` | Three-trait colocalization of PD GWAS with cell-type caQTL and eQTL signals at lead risk loci |
| 07 | `GWAS-LDSC-QTL.sh` | Stratified LD-score regression: enrichment of PD heritability in cell-type QTL annotations |
...
| Step | Script | Biological aim |
|------|--------|----------------|
| 08 | `PD-differential-nebula-deg.R` | Single-cell DEGs between PD, ILBD, and HC within each cell type (NEBULA mixed models), adjusting for age, sex, PMI, batch, and technical factors |
| 09 | `scMultiMap-peak-gene.R` | Link accessible peaks to candidate target genes in *cis* (±1 Mb) per cell type |
...
| Step | Script | Biological aim |
|------|--------|----------------|
| 10.1 | `scenicplus_step1_per_celltype.py` | Topic modeling of accessibility (pycisTopic), differential accessible regions, and gene activity per cell type |
| 10.2 | `scenicplus_step2_per_celltype.py` | Motif / cisTarget databases for cell-type region sets |
| 10.3 | `scenicplus_step3_per_celltype.py` | Configure SCENIC+ GRN inference (enhancer-driven eRegulons) |
| 11 | `eRegulon-GSEA-core.R` | Test whether SCENIC+ eRegulon target genes are enriched among diagnosis DEG rankings |
