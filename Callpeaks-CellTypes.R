# =============================================================================
# Step 03 - MACS peak calling, union peak set, and cell-type-specific peaks
# -----------------------------------------------------------------------------
...
## ---- Cell-type-specific peaks (presence-based) ------------------------------

specific_dir <- file.path(DIR_MACS, "cell_type_specific_peaks/CellSpecifityExist")
dir.create(specific_dir, recursive = TRUE, showWarnings = FALSE)

cell_type_columns <- rev(c("Astrocytes", "Oligodendrocytes", "OPCs", "Microglia", "GABA_neurons",
                           "Glu_neurons", "DA_neurons", "Macrophages", "T_Cells",
                           "Endothelial_cell", "Pericytes"))
cell_type_matrix <- as.data.frame(mcols(union_peaks)[, cell_type_columns])
# A peak is specific if it is present in exactly one cell type.
specific_peaks <- apply(cell_type_matrix, 1, function(row) {
  non_zero_indices <- which(row > 0)
  if (length(non_zero_indices) == 1) non_zero_indices else NA
})
union_peaks$specific_cell_type <- cell_type_columns[specific_peaks]
union_peaks$specific <- !is.na(union_peaks$specific_cell_type)

union_peaks2 <- union_peaks[union_peaks$signalValue > 2, ]
specific_peaks_valid <- union_peaks2[!is.na(union_peaks2$specific_cell_type) & union_peaks2$specific]
specific_peaks_list <- split(specific_peaks_valid, specific_peaks_valid$specific_cell_type)
saveRDS(specific_peaks_list, file.path(specific_dir, "cell_type_specific_peaks_list.rds"))
print(sapply(specific_peaks_list, length))
